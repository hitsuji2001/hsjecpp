...
Instance::Instance(const std::string& app_name,
  ...
  const auto& create_info =
      this->create_instance_info(app_info, required_extensions);

  vk::check_result(vkCreateInstance(&create_info, nullptr, &this->instance),
                   "Failed to create Vulkan Instance");
#ifdef _DEBUG
  log::debug("Creating Debug Messenger...");
  vk::check_result(this->create_debug_utils_messenger_ext(
                       this->instance, &this->debug_create_info, nullptr,
...
Instance::~Instance() {
  ...
  this->destroy_debug_utils_messenger_ext(this->instance, this->debug_messenger,
                                          nullptr);
...
VkInstance Instance::get() const { return this->instance; }

VkApplicationInfo Instance::create_app_info(
    const std::string& app_name, const std::string& engine_name) const {
...
VkInstanceCreateInfo Instance::create_instance_info(
  ...
  create_info.pNext =
      (VkDebugUtilsMessengerCreateInfoEXT*)&this->debug_create_info;
#else
...
Instance::create_debug_messenger_create_info() const {
...
VKAPI_ATTR VkBool32 VKAPI_CALL Instance::debug_callback(
  ...
  const std::string message = fmt::format("[{} - {}]: {}", "Validation Layer",
