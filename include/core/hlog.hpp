...
class log {
 ...
 private:
  ...
  template <typename... T>
  static void _print_log(Severity severity, fmt::format_string<T...> format,
                         T&&... args) {
    ...
    fmt::print(" ");
    fmt::print(_severity_text_styles.at(severity), "[{:^5}]",
               _severity_names.at(severity));
    fmt::print(" ");
 ...
 private:
  ...
  static const std::unordered_map<Severity, const fmt::text_style>
      _severity_base_color;
  static const std::unordered_map<Severity, const fmt::text_style>
      _severity_text_styles;
  static const std::unordered_map<Severity, const fmt::text_style>
      _severity_message_styles;
};
}  // namespace hlog
